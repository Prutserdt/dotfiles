...
FIXME: onderstaande is klad

verander de naam van mijn systemen.
mijn desktop, genaamd linuxbox, ga ik NIET veranderen. wel:

omschrijving        system-name oud     system-name nieuw
Virtual box Arch    ArchLinux
Thinkpad Arch       archlinux
...
#+begin_src shell
sudo hostnamectl set-hostname yolo
#+end_src
...
The following part is system specific. Is my system name is =archlinux= then my computer is a laptop and I want to show battery information in my top screen. If my computer is a desktop, and not named =archlinux= then I do not want to show the battery information.

FIXME: REMOVE LATER, LEAVE FOR NOW...
#+BEGIN_SRC python
