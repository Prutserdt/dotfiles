...
** Emacs Python script
The next piece of code is written to a separate script, named =open_emacs.py= and is used to open emacs files. A note is added to ~open_emacs.py~ which reminds *not* to edit this file directly.
