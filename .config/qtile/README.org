...
** Variables
Some variables are declared. The left super key, right super key and right alt key are used for keybindings. Emacs variables for opening files in these editors.
#+BEGIN_SRC python :tangle config.py :results none
...
** Emacs Python script
The next piece of code is written to a separate script, named =open_emacs.py= and is used to open emacs files. First the required libraries are imported.
...
The code execution starts here. The code will run if the filename is a single one
#+BEGIN_SRC python :tangle open_emacs.py
if __name__ == "__main__":
    if len(sys.argv) != 2:
 #       print("Usage: python3 open_emacs.py <FILENAME>")
        sys.exit(1)
...
onderstaande is oude code, weghalen als het werkt....
#+BEGIN_SRC python
import subprocess
import sys
...
def is_emacsclient_running():
    try:
        subprocess.check_output(["emacsclient", "-e", "(message \"Emacs is running\")"])
        return True
    except subprocess.CalledProcessError:
        return False
