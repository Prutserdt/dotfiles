...
- [[#about-qtile-and-my-config][About qtile and my config]]
- [[#imports][Imports]]
- [[#variables][Variables]]
- [[#keybindings-qtile-specific][Keybindings: qtile specific]]
- [[#keybindings-audio][Keybindings: Audio]]
...
* About qtile and my config
[[http://www.qtile.org/][Qtile]] is a full-featured, hackable tiling window manager written and configured in Python.
In this emacs org document my configuration of qtile is described and it is used as the source to create the qtile configuration file [[https://github.com/Prutserdt/dotfiles/blob/master/.config/qtile/config.py][config.py]].

* Imports
This Python configuration requires some modules to be loaded.

#+BEGIN_SRC python :tangle "configFROMorg.py"
import psutil
import subprocess
import os
...
#+BEGIN_SRC python :tangle "configFROMorg.py"
    ...
    Key([altR], "v",
        ...
        desc="Open in vim:"
         is exclusively used to Key([altR], "w",
...
layouts = [    layout.MonadThreeCol(**layout_theme, min_ratio=0.05, max_ratio=0.9,
...
widget_defaHotkeys to control the volume and mute.ults = dict(
    font="hack",
    fontsize=12,
    padding=3,
)
...
This shellscript is starting some keyboard specific scripts for my redox keyboards and start an Emacs daemon.
