...
** Imports
My configuration requires some modules to be loaded, all are default settings.
...
** Variables
Some variables are declared. The left super key, right super key and right alt key, which I use for keybindings. Also the terminal and opening of VIM and Emacs are declared.
...
** Custom functions
Here I created some functions. These are used for keybinding and are utilized in the next keybinding section.
...
My left super key bindings are all related to qtile and do thing like moving windows around.
