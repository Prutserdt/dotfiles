...
A NOTE is added to ~config.el~ remember to *not* edit this file directly.
...
def testt(qtile):
    qtile.cmd_to_layout_index(6) #3: monadwide with margins
...
A NOTE is added to ~autosart.sh~ to remember *not* to edit this file directly.
