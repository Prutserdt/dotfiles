...
This is a literate programming document and it describes my Qtile window manager customization and is the source of my configuration as well. The Emacs package ~org-babel-tangle~ exports the [[https://orgmode.org/manual/Extracting-Source-Code.html][code blocks]] to the actual configuration files. This information might not be important to you and the files, [[https://github.com/Prutserdt/dotfiles/blob/master/.config/qtile/config.py][config.py]], [[https://github.com/Prutserdt/dotfiles/blob/master/.config/qtile/open_emacs.py][open_emacs.py]] and [[https://github.com/Prutserdt/dotfiles/blob/master/.config/qtile/autostart.sh][autostart.sh]] can be checked individually as well.
...
It is possible to check the source code of Qtile if it installed on your system by running from python shell: ~import libqtile~ and start by ~print(help(libqtile))~ and ~dir(libqtile)~.
