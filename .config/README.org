...
Please write a shell script that will open ~/.config/doom/README.org in emacs. It should first check if the daemon is running (emacs --daemon) and if it is not running then first start it and then open the file in emacs by running emacsclient -c ~/.config/doom/README.org.

#+BEGIN_SRC bash :tangle emacs_open_file.sh :padline no :eval no :tangle-mode (identity #o755)
#!/bin/bash
emacsclient -r ~/.config/doom/README.org
#+END_SRC
