...
** emacs
A script to quickly open a file in emacs. I want to use emacs by using emacsclient. This should first check if =emacs --daemon= is running and if so then run =emacsclient -c filename=.

Please write a shell script that will open ~/.doom.d/README.org in emacs. It should first check if the daemon is running (emacs --daemon) and if it is not running then first start it and then open the file in emacs by running emacsclient -c ~/.doom.d/README.org.

#+BEGIN_SRC bash :tangle emacs_open_file.sh :padline no :eval no :tangle-mode (identity #o755)
#!/bin/bash
emacsclient -r ~/.doom.d/README.org
#+END_SRC
...
under construction, right now playing with this script to modify some keys dependent on my hostname.

#+BEGIN_SRC sh :tangle keyboard-config.sh :padline no :eval no :tangle-mode (identity #o755)
#!/bin/bash
# NOTE: This file is generated from ~/.config/README.org
#       Please only edit that file and org-babel-tangle (emacs)
...
NOTE: onderstaande is oud en ga ik weghalen!
...
I like to use the Super-R button but this is handled different on my systems. On my Redox keyboard I put in in my firmware and it can be used by keycode 134 and in my thinkpad I will use the printscreen button as the Super-R button (keycode 134). This switching is automated according to the system name.
...
if [ "$system_name" = "archlinux" ]; then
    keycode 107 = Super_R
elif [ "$system_name" = "linuxbox" ]; then
    keycode 134 = Super_R
else
    notify-send -t 60000 "Unknown system name: $system_name"
    exit 1
