...
- [[#my-scripts][My scripts]]
  ...
  - [[#decrmonitorbrightness][decrMonitorBrightness]]
  ...
  - [[#incrmonitorbrightness][incrMonitorBrightness]]
  - [[#kbswitch][kbswitch]]
  - [[#notify-logsh][notify-log.sh]]
  - [[#unicode-how-to-create-a-list-of-emojis-used-in-dmenuunicodesh][unicode. How to create a list of emojis (used in dmenuunicode.sh)]]
  - [[#rdxswitch][rdxswitch]]
...
* About this document
This document is a note-to-self about scripts on my GNU/Linux system and is my attempt at literate programming using Emacs org mode. This file is situated in my $HOME/.config and named as README.org so that it will be recognized by github as the title page.
...
** decrMonitorBrightness
Decrease the monitor brightness by xrandr.
...
First the current brightness is selected by awk. Make sure that the right output device is chosen, for instance ~HDMI-2~. If the value is set to 0.10 then it will not decrease it any further and making the screen completely black.
#+begin_src sh :tangle decrMonitorBrightness.sh :padline no :eval no :tangle-mode (identity #o755)
...
if [[ "$currentBrightness" == "0.10" ]] ; then
    ...
    xrandr --output HDMI-2 --brightness $newBrightness
...
** incrMonitorBrightness
Increase the monitor brightness by xrandr.
...
First the current brightness is selected by awk. If the value is set to 1.0 then it will not increase it any further and possibly damaging the screen. Make sure that the right output device is chosen, for instance ~HDMI-2~.
#+begin_src sh :tangle incrMonitorBrightness.sh :padline no :eval no :tangle-mode (identity #o755)
currentBrightness=$(xrandr --verbose | awk '/Brightness/ { print $2; exit }')
echo $currentBrightness
...
** notify-log.sh
This script will write the notifications to a logfile.
...
** unicode. How to create a list of emojis (used in dmenuunicode.sh)
