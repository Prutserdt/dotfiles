...
* About this document
This org document is a literate programming document about the scripts that I use on my GNU/Linux computer. This file is situated in my $HOME/.config and named as README.org so that it will be recognized by github as the title page.
...
#+begin_src sh :tangle dotfiles_.sh :padline no :eval no
#!/bin/bash
#+end_src
...
The next code block will give the output of all of the scripts of the ~/.config as separate code blocks. Very handy to import all of the scripts.
