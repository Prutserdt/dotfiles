#+TITLE: Repository of all of my scripts
#+auto_tangle: t
#+AUTHOR: Prutserdt
...
#+BEGIN_SRC bash :tangle dmenuemacs.sh  :padline no :tangle-mode (identity #o755) :eval never
notify-send -t 1000 "Running dmenuemacs.sh" &&
...
#+BEGIN_SRC bash :tangle dmenukill.sh  :padline no :tangle-mode (identity #o755) :eval never
notify-send -t 1000 "Running dmenukill.sh, a script to kill processes!" &&
...
case "$selected_option" in
    "d Delete process")
        kill "$chosen"
        notify-send -t 1000 "The process '$process_name' with PID '$chosen' was killed!"
        ;;
    "c Copy process name")
        echo "$process_name" | xclip -selection clipboard
        notify-send -t 1000 "Process name '$process_name', coupled to PID '$chosen', was copied to clipboard!"
        ;;
    "p Copy PID")
        echo "$chosen" | xclip -selection clipboard
        notify-send -t 1000 "Selected PID '$chosen', coupled to process name '$process_name', was copied to clipboard!"
        ;;
    "e Exit")
        notify-send -t 1000 "Exiting dmenukill.sh"
        exit
        ;;
esac
#+END_SRC
...
** dmenuthunar.sh
A dmenu script to open directories in pcmanfm. Yes, the name is incorrect and thunar is not used, but I keep it like this for now.

This script should start with a directive, first a shebang ~#!~ and to be POSIX compliant I choose ~sh~ here.
...
I am currently creating the directories file located here:
#+BEGIN_SRC emacs-lisp :results none
(find-file "~/Stack/Command_line/directories")
#+END_SRC
And copy the content of this gtk directory to hte directories file(with slight modifications):
#+BEGIN_SRC emacs-lisp :results none
(find-file "~/.config/gtk-3.0/bookmarks")
#+END_SRC
...
image_files=$(find . -maxdepth 1 -type f -iname "*.jpeg" -o -iname "*.jpg" -o -iname "*.png" -o -iname "*.gif" | sort) && selected_image=$(echo "$image_files" | nsxiv -ftio) && [ -n "$selected_image" ] && exif_info=$(exiftool "$selected_image" | grep "GPS Position") && [ -n "$exif_info" ] && gps_position=$(echo "$exif_info" | awk -F ": " "{print $2}" | tr -d " " | sed "s/deg/°/g") && maps_url="https://www.google.com/maps/place/$gps_position" && xdg-open "$maps_url" && notify-send -t 1000 "Google maps opened with GPS location: $gps_position" || echo "no exif data in picture"
#+end_src
