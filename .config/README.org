...
- [[#tips-and-tricks][Tips and tricks]]
  ...
  - [[#set-file-rights-back-chmod][Set file rights back (chmod)]]
...
Any script should start with a directive, first a shebang ~#!~ and to be POSIX compliant I choose ~sh~ here.
#+begin_src bash :tangle dmenupatch.sh :padline no
#!/bin/sh
#+end_src
...
Hunk #1 FAILED at 2.
Hunk #2 succeeded at 25 with fuzz 2 (offset 12 lines).
1 out of 2 hunks FAILED -- saving rejects to file config.def.h.rej
patching file dmenu.c
Hunk #1 succeeded at 11 (offset 1 line).
Hunk #2 succeeded at 27 with fuzz 2 (offset 1 line).
Hunk #3 succeeded at 60 (offset 4 lines).
Hunk #4 succeeded at 862 (offset 251 lines).
Hunk #5 FAILED at 649.
Hunk #6 FAILED at 666.
Hunk #7 succeeded at 1036 (offset 280 lines).
Hunk #8 succeeded at 1059 (offset 280 lines).
2 out of 8 hunks FAILED -- saving rejects to file dmenu.c.rej
patching file drw.c
Hunk #4 succeeded at 191 (offset -6 lines).
Hunk #5 succeeded at 216 (offset -6 lines).
Hunk #6 succeeded at 272 (offset -6 lines).
patching file drw.h
...
** Set file rights back (chmod)
After running the codeblocks the shell scripts are no longer executable. The following script will make all of the *.sh and *.py files in ~/.config executable again.
