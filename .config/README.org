...
Decrease the monitor brightness by xrandr. It's brother, [[incrMonitorBrightness.sh]], is a similar script doing the oposite.
...
Increase the monitor brightness by xrandr. It's sister, [[decrMonitorBrightness.sh]], is a similar script doing the oposite.
...
First the current brightness is selected by awk. If the value is set to 1.0 then it will not increase it any further and prevent damaging the screen. Make sure that the right output device is chosen, for instance ~HDMI-2~.
#+begin_src sh :tangle incrMonitorBrightness.sh :padline no :eval no :tangle-mode (identity #o755)
currentBrightness=$(xrandr --verbose | awk '/Brightness/ { print $2; exit }')
echo $currentBrightness
...
** nightColorDown.sh
A script to make the screen more reddish.
...
** resetRGB.sh
Reset the gamma settings (RGB) to a default value of 1:1:1.
