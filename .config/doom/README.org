...
- [[#configel][config.el]]
  ...
  - [[#run-python-test_codepy][Run Python test_code.py]]
  ...
  - [[#render-image-of-smiles][Render image of SMILES]]
...
** Render image of SMILES
This function will render an image from point by open-babel and SMILES code. Go to a line with a single SMILES line and run this functin to display a picture of the molecular structure with nsxiv inside of Emacs. The image is temporarily stored in the systems RAM.
#+begin_src elisp :results none
(defun my-obabel-render-image-and-show ()
  "Show molecular structure from SMILES code at point"
  (interactive)
  (let ((smiles (thing-at-point 'line)))
    ;;(let ((output-file "/home/icefly/Downloads/output_file.png"))
    (let ((output-file "/dev/shm/temp/"))
      ;;(shell-command (concat "obabel -xb -:" (shell-quote-argument smiles) " -O " (shell-quote-argument output-file) " -xp 400 &") "/obabel-output/")
