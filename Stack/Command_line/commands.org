...
#+begin_src
 _     _                    _   _                             _
| |   (_)_ __  _   ___  __ | |_(_)_ __  ___    __ _ _ __   __| |
| |   | | '_ \| | | \ \/ / | __| | '_ \/ __|  / _` | '_ \ / _` |
| |___| | | | | |_| |>  <  | |_| | |_) \__ \ | (_| | | | | (_| |
|_____|_|_| |_|\__,_/_/\_\  \__|_| .__/|___/  \__,_|_| |_|\__,_|
                                 |_|
 _        _      _
| |_ _ __(_) ___| | _____
| __| '__| |/ __| |/ / __|
...
- [[#powerstrike-punt-nl][powerstrike punt nl]]
...
* About this document
This document consists of notes that I took during my Linux journey. A *lot* of them are obsolete, faulty and inefficient. I wrote things down to memorize them better and to keep as a reference. Warning, the file is larger than 4800 lines!
...
*** cowsay. Most important feature of Linux.
...
Some options:
fortune | cowsay | lolcat # Let the cow quote! (requires conwsay and fortune-mod)
fortune | cowsay -W 120 -f eye | lolcat
fortune | cowsay -W 120 -d | lolcat
fortune | cowsay -W 120 -b | lolcat
fortune | cowsay -W 120 -g | lolcat
fortune | cowsay -W 120 -p | lolcat
fortune | cowsay -W 120 -s | lolcat
fortune | cowsay -W 120 -t | lolcat
fortune | cowsay -W 120 -w | lolcat
fortune | cowsay -W 120 -y | lolcat
fortune | figlet -w 120 | lolcat
fortune | figlet -w 120 | lolcat
fortune | cowsay -W 120 -d | lolcat


*** Default applicaties in Linux. Bijvoorbeeld de browser.

Brave kun je default browser maken vanuit brave (hamburger menu, settings, 'make browser the default'.).
Dit wordt ingesteld in een dotfile: ~/.config/mimeapps.list
Firefox:
#+begin_src shell
[Default Applications]
text/html=userapp-Firefox-M09F31.desktop
x-scheme-handler/http=userapp-Firefox-M09F31.desktop
x-scheme-handler/https=userapp-Firefox-M09F31.desktop
...
*** Show the keys that are pressed on the screen: screenkey
A handy tool to show the key presses is screenkey. keylogger keys strokes
...
*** Foto's van bezoek Wenen

Eerst de volgende command line commandos uitgevoerd:
~exiv2 -r'%Y%m%d-%H%M_:basename:' rename $(ls)~
De foto's van mijn Pixel worden niet renamed. De naam is wel bijna juist, voorbeeld: IMG_20230123_130402_528.jpg. Wat ik wil doen is voor alle files in directory genaamd IMG_* de eerste vier tekens verwijderen en verder met de eerste underscore veranderd worden in een hyphen.

#+begin_src shell
for filename in IMG_* ; do
    echo "$filename"
    newName=${filename:4} # remove first four characters IMG_
    echo "$newName"
    echo $newName | sed 's/./-/9' # change underscore at position 9 to hyphen
    # Up to here nothing is changed. Uncomment the next line to change the name(s).
    #mv "$filename" $(echo $newName | sed 's/./-/9')
done
#+end_src

Rename *.JPG to *.jpg: ~find -name '*.JPG' -exec rename .JPG .jpg {} \;~

Nu nog bestemming toevoegen:
#+begin_src shell
rename 20230122 20230122_Arnhem_naar_Wenen *.jpg
rename 20230123 20230123_Wenen *.jpg
rename 20230124 20230124_Wenen *.jpg
rename 20230125 20230125_Wenen *.jpg
#+end_src

Daarna het bovenstaande samengevoegd tot een enkel commando:
#+begin_src shell
rename 20230122 20230122_Arnhem_naar_Wenen *.jpg;rename 20230123 20230123_Wenen *.jpg;rename 20230124 20230124_Wenen *.jpg;rename 20230125 20230125_Wenen *.jpg
#+end_src
...
*** Vakantie Bali 2023
Camera aangesloten via USB kabel en aangezet en gemount met: ~sudo mount /dev/sdc1 /mnt/USBdrive~.
Foto's in de tijdelijke map ~/Downloads/2023_Bali_raw_data~ gezet.
Ik heb nog even gekeken of ik de namen automatisch kan laten genereren via GPS, maar er staat geen GPS informatie in mijn Sony alpha-7 camera, dus maar niet gedaan. Dit is te checken via ~exiftool picturename.jpg | grep GPS~.


De apple fotos van drie devices heb ik verkregen downloadable albums, net zoals de , maar dan meer devices.

FIXME: klad staat hieronder
...
Check info van package: ~pacman -Qi qtile~
...
*** Bluetooth instellen 2023
Install: ~sudo pacman -S bluez bluez-utils~
Run the daemon: ~sudo systemctl enable bluetooth.service~
Check status: ~rfkill~
Er is geen output, geen bluetooth en wireless lijkt het...
Install: ~sudo pacman -S blueman~

Je moet als user rechten hebben voor de ~lp~ groep:
Check of lp erbij staat: ~groups icefly~
Nee, geen lp...
Toevoegen van lp via: ~sudo gpasswd -a icefly lp~
Check of lp erbij staat: ~groups icefly~
yes 😃.

commandline ~bluetoothctcl~ werkt niet voor pairing helaas.
...
side control sliding knee toevoegen!
opstaan uit guard sleeve toevoegen
...
for element in videolijst:
    commando=("alacritty -e youtube-dl --verbose '" + element + "'")
    print(commando)
    os.system(commando)
#+end_src

#+RESULTS:
: Data wordt hier weggeschreven: /home/icefly/Stack/Command_line
: alacritty -e youtube-dl --verbose 'https://player.vimeo.com/video/137042644?h=53cbd86d0b&dnt=1&app_id=122963'
: alacritty -e youtube-dl --verbose 'https://player.vimeo.com/video/136989993?h=f50eb9223c&dnt=1&app_id=122963'
...
*** ZSH en Oh-my-zhs
Na oh-my-zsh-git installatie via curl commando dat op github staat maak een config via:
...
*** Virtualbox (mei 2023)
Na installatie van virtualbox op het werk op mijn Windows laptop en installatie van de extension pack kreeg ik uiteindelijk shared directory aan het werk. Eerst was het in een root directory met root rights, wat onhandig is. Ik heb de vboxsf groep toegevoegd aan de user door: ~sudo usermod -G vboxsf -a username~ en een reboot
...
*** Transform image to ascii art
~ascii-image-converter~ will do as advertized and the -n flag makes it negative and the -W flag sets the width.

#+begin_src sh :results output
ascii-image-converter -n -W 60 ~/Downloads/Logitech_mouse.jpg
#+end_src
...
*** ncmpcpp. Weer een andere terminal music player
Na ~sudo pacman -S ncmpcpp~ stond er geen ~~/ncmpcpp~ configuratie dus die gekopieerd via: ~cp /usr/share/doc/ncmpcpp/config config~

Bovenstaande werkt niet. Blijkt dat mpd geinstalleerd moet worden! Geen depency was het !
~cp //etc/mpd.conf ~/.config/mpd~

En het is een systemd service: ~systemctl start mpd.service~ .
De service werkt...
ncmpcpp werkt nog niet...
...
*** Working with an extra github repo

Created an open repo https://github.com/Prutserdt/PowerStrike_code
Change the current working directory to the location where you want the cloned directory.
~cd ~/Stack/Code/git~
Clone git: ~git clone https://github.com/Prutserdt/PowerStrike~
Edit the README.org file and upload by:
~git add .
~git commit -m 'Test commit'~
~git push origin main~
foutmelding, probeerd via https aan te melden wat niet meer gaat, op te lossen door:
go to the .git directory and change the config file:
url = ssh://git@github.com/Prutserdt/PowerStrikedot
...
Change the current working directory to the location where you want the cloned directory.
~cd ~/Stack/Code/git~
Clone git: ~git clone ssh://git@github.com/Prutserdt/PowerStrike_code~
Het gaf een melding dat de repo empty is, dat klopt 😀

~git add .
~git commit -m 'Upload of the first files to this repo'~
~git push origin main~
...
Daarna steamcmd van geinstalleerd de AUR
yay steamcmd
...
*** Problemen met vinden van ttyUSB
Check met ~lsusb~ en ~lsusb -t -v~.
Probleem opgelost na een reboot.
...
| alias       | description                                          |
|-------------+------------------------------------------------------|
| em          | run emacs in commandline                             |
| emacs       | run emacs graphically                                |
| doomdoctor  | diagnose issues of emacs and config                  |
| doomsync    | synchronize private config                           |
| doomupgrade | update Doom to latest release and installed packages |
| doombuild   | recompile all packages, typically after upgrade      |
| doompurge   | delete old orphaned packages                         |
| doomhelp    | help                                                 |
|-------------+------------------------------------------------------|

Na een upgrade van mijn configuratie op mijn tweede machine werkte emacs niet meer, er was een missende .el file in de /.emacs.d directory . Dit is eenvoudig opgelost door de org file te saven, daarna ~~/.doom.d~ en ~~/.emacs.d~ te verwijderen en daarna een nieuwe installatie uit te voeren volgens de Doom emacs Github installatie omschrijving (~git clone --depth 1 --single-branch https://github.com/doomemacs/doomemacs ~/.config/emacs ~/.config/emacs/bin/doom install~). Daarna de org file terug zetten en deze tanglen, et viola!


**** Allerlei Lisp functies

Doornummeren van een visual selection block: ~rectangle-number-lines~
...
**** Vul selectie met oplopende nummers. Fill selection with increasing numbers.
Maak bijvoorbeeld met CTL-V een vertikale selectie en plak daar oplopende nummers is met ~rectangle-number-lines~.
...
***** TODO mu4e installation and settings
...
[ [link] [link omschrijving]]: [[duckduckgo.com][de beste zoek website]]. Dit kan ook via ~org-insert-link~.
[[img:asdf.jpg][asdf]
...
Je kunt regels/kolommen verschuiven met Meta pijltjes 🤩.
...
***** File rights after tangling (executable)

Make the tangled code block executable by adding ~~tangle-mode (identity #o755)~ to the begin of the block, for example: ~#+begin_src bash :tangle dmenuapps.sh :padline no :tangle-mode (identity #o755).~ Now the file has executable rights.
...
Let op, als je wilt dat de code _NIET_ wordt gewrapped in ~int main() {~ dan moet je ervoor zorgen dat er ~:main no~ komt te staan in de bovenste regel.
#+begin_src C++ :tangle PowerStrike_code.ino :main no :results raw
/* -*- c -*- */
/*********
wat ik wil: connecten via accesspoint naar esp32 vanuit mobieltje
Dan inloggen als gebruiker en beginnen met workout die wordt opgeslagen
#+end_src


***** Arduino, .ino debugging: via code block

#+begin_src shell
arduino --verify ~/Stack/Code/git/PowerStrike_code_Adafruit_IO/PowerStrike_code_Adafruit_IO.ino
#+end_src

***** Arduino.ino debugging. Snelle debugging in emacs via elisp

Het volgende elisp code block runt een asynchroon shellscript ~arduino --verify code.ino~~. Dit zorgt ervoor dat deze in een apart proces loopt en je emacs kunt blijven gebruiken. De debug output is te lezen in de buffer ~*Async Shell Command*~. Verder wordt deze buffer netjes aande de linkerkant gezet.Een mooie elegante oplossing voor debugging.

#+NAME: debug
#+begin_src elisp
(async-shell-command "arduino --verify ~/Stack/Code/git/PowerStrike_code_Adafruit_IO/PowerStrike_code_Adafruit_IO.ino")
...
Laat de inhoud van directory zien
#+begin_src shell
cd ~/Downloads/test/C*;
ls
#+end_src
...
#+RESULTS:
| ELN_upload_file.xlsx          |
| JUS2-00046_01MAR23-AEX_01.csv |
| JUS2-00046_01MAR23-AEX_02.csv |
| JUS2-00046_01MAR23_01.csv     |
| JUS2-00046_27FEB23_UFDF.csv   |
| JUS2-00046_27FEB23_UFDF_1.csv |
| JUS2-00046_27FEB23_VF.csv     |
| JUS2-00046_27FEB23_VF_1.csv   |
| JUS2-00046_27FEB23_runIX.csv  |


Zoek regel met zoekterm in alle csv files
#+begin_src shell
cd ~/Downloads/test/C*;
grep -i "Dataset Name" *.csv; # show line with name in CSV file
#+end_src
...
Test with counting line numbers:
#+NAME: wordcounting
#+begin_src elisp :result value org
(count-lines-page)
#+end_src

#+RESULTS: wordcounting
: Page has 4807 lines (3360 + 1448)
...
Zie ook de introductie paragraaf van dit bestand, daar wordt ~#+CALL: woordtellen()~ gebruikt.

#+CALL: wordcounting()

#+RESULTS:
: Page has 4807 lines (3389 + 1419)
...
*** Show the keys that are pressed on the screen: screenkey
A handy tool to show the key presses is screenkey.

*** Home assistent, hacs
Installed via ~yay home-assistant-hacs~~. Verder niet gebruikt


* powerstrike punt nl
03feb23: wordpress aangemaakt.
wordpress 6.1.1
WP Data Access plugin geinstalleerd.
...
*** 01APR23: install Arch linux on Lenovo ThinkPad X270

Arch iso via usb connected. Connectie met internet via ~iwctl~ gemaakt en daarna ~arch-install~ doorheen gelopen voor de base install.

Packages die ik nodig had:
~yay python-dbus-next feh xev gimp thunar acpi lxappearance ark sxiv screenkey brave alacritty git base-devel xfce4 sfce4-goodies gvim alsa-utils pfetch-git cowsay fortune-mod ttf-hack emacs ripgrep fd picom numlockx pywal python-psutil oh-my-zsh keepass gtk4 libgtk2 gtk2 qalculate-gtk python-pip arduino arduino-cli mpv xclip~

pip install thefuck

- Battery monitor is added to qtile, the following three parts should be uncommented in the config files that is generated from the README.org.
...
Op de Lenovo wil ik de PrtSc toets gebruiken voor Super_L. Dit heb ik commented aan mijn rdxswitch toegevoegd.

- Networkmanager install and qtile config
  ~sudo pacman -S networkmanager network-manager-applet python-iwlib. Start service: ~systemctl start NetworkManager.service~.
  Check if a static hostname is set, this is needed for networkmanager: ~hostnamectl~. If needed set the Static hostname to in this example linuxbox: ~hostnamectl set-hostname linuxbox~
  Set the ~sudo systemctl enable NetworkManager.service~. Add to the qtile startup.sh: ~nm-applet~.


* Keyboard settings
I prefer custom keyboard settings, for instance swap escape/capslock and use left and right CTRL/Super/Alt keys. This can be set by xmodmap but this resulted in a config that was not stable, and switched back to default regularly. Therefore I switched to XKB.

TODO: use an alternative to xmodmap
*** xmodmap
This configuration was *not* stable for me, I am not using it anymore and the next part is just for reference in case I need it in the future.
...
*** dmenu, upgrade Jan2023, dmenu-distrotube

**** Install
Downloaded the gitlab repo, unpacked, backup gemaakt van mijn persoonlijke huidige dmenu via dmenubackup.sh.

sudo make clean install && rm config.h

Het onderstaande bash script verwijderd de huidige dmenu versie en schrijft distrotube versie weg.
#+begin_src bash
rm -r ~/.config/suckless/dmenu &&
mkdir ~/.config/suckless/dmenu &&
cp -r ~/Downloads/dmenu-distrotube-master/dmenu-distrotube-master/* ~/.config/suckless/dmenu
...
Daarna de installatie gedaan maken met ~sudo make clean install~ en daarna ~rm ~/.config/suckless/dmenu/config.h~.

Ik heb een tokyo-night.h file toegevoegd in de colors directory in het orgineel op de download locatie.
...
*** Logitech G502 hero mouse, configuration on Lixux

Mouse setting modified with ~piper~ (GUI for LibRatBag). This happens directly from the device (read/write), there are no dotfiles.

Current sensitivity: 500 Hz, 12500 DPI.
...
#+NAME: Logitech_G502_mouse_numbering
#+begin_example
...
Profile0
|--------+---------------------------+---------------------------+---------------------------|
| button | location of button        | button/macro              | Aim/description of use    |
|--------+---------------------------+---------------------------+---------------------------|
|      1 | left mouse                | linkermuisknop            | left mouse button         |
|      2 | right mouse               | rechtermuisknop           | right mouse button        |
|      3 | scroll wheel              | middle mouse click        | middle mouse button       |
|      4 | right thumb back          | Modifier4                 | copy                      |
|      5 | right thumb forward       | Modifier5                 | paste                     |
|      6 | right thumb tip           | Escape                    | Escape emacs modes        |
|      7 | left of left finger, tip  | y                         | emacs evil mode yank      |
|      8 | left of left finger, back | p                         | emacs evil mode paste     |
|      9 | scroll wheel left tilt    | leftSuper+1               | move to first workspace   |
|     10 | scroll wheel right tilt   | leftSuper+4               | move to fourth workspace  |
|     11 | below scroll wheel        | Logitech Cycle Profile up | Logitech Cycle Profile up |
|--------+---------------------------+---------------------------+---------------------------|

Profile1
|--------+---------------------------+---------------------------+---------------------------|
| button | location of button        | assigned keypress/macro   | Aim/description of use    |
|--------+---------------------------+---------------------------+---------------------------|
|      1 | left mouse                | left mouse                | left mouse button         |
|      2 | right mouse               | right mouse               | right mouse button        |
|      3 | scroll wheel              | middle mouse              | middle mouse button       |
|      4 | right thumb back          | Modifier4                 | go back page browser      |
|      5 | right thumb forward       | Modifier5                 | go forward page browser   |
|      6 | right thumb tip           | control+W                 | close tab in browser      |
|      7 | left of left finger, tip  | control+c                 | copy                      |
|      8 | left of left finger, back | control+v                 | paste                     |
|      9 | scroll wheel left tilt    | leftcontrol+Pageup        | move tab left             |
|     10 | scroll wheel right tilt   | leftcontrol+Pagedown      | move tab right            |
|     11 | below scroll wheel        | Logitech Cycle Profile up | Logitech Cycle Profile up |
|--------+---------------------------+---------------------------+---------------------------|
