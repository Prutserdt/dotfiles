...
To position your Emacs window in the middle of the screen you can employ visual-fill. This configuration is especially useful for maximized windows on an ultrawide monitor, providing a clean and centered workspace.
#+BEGIN_SRC emacs-lisp :tangle config.el :results silent
...
Set multilingual text input to ~latin-prefix~ as the default. This will modify ~Ol'e~ input to ~Olé~, ~//~ to ~°~ and =~e= to ~€~. Sometimes this is not the desired behaviour and therefor the function =my-switch-input-method= can be used to exit out of the multilingual input. The function is mapped to an evil keybinding (SPC d i).
#+BEGIN_SRC emacs-lisp :tangle config.el :results silent
