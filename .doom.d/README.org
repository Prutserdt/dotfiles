...
NOTES: some packages should be installed on the system, separate from the doom configuration:
=sudo pacman -S python-pytest python-nose=
...
I want to use the =undo-redo= package in evil mode and therefore I add the keybinding to =U= here.
#+BEGIN_SRC emacs-lisp :tangle config.el :results silent
(after! evil
...
** Custom keybindings
Emacs uses a lot of keybindings and Doom Emacs adds even more on top of it. Next to that I add a few myself.
#+BEGIN_SRC emacs-lisp :tangle config.el :results silent
(map! :leader
...
Often I get into the wrong keycord and want to exit quickly. This is possible by =C-g= but I think =Esc= is more intuitive and I will change the default Doom Emacs behaviour. Let's see if this will give conflicts somewhere else. NOTE: in case of conflicts then a custom function can be used to only use this behaviour once a keychord is active.
#+BEGIN_SRC emacs-lisp :tangle config.el :results silent
(global-set-key (kbd "<escape>")      'keyboard-escape-quit)
#+END_SRC

** Org-mode configuration
Setup to use sql in org code blocks. Commented out because I am not using it at the time.
#+BEGIN_SRC emacs-lisp :tangle config.el :results silent
...
** Custom code
*** Custom elisp funtions (used with hotkeys)
...
Comparing my org files from three different directories, wow, seems complicated to compare files....
#+BEGIN_SRC emacs-lisp :tangle config.el :results silent
...
Switch back to the default roam-notes.
#+BEGIN_SRC emacs-lisp :tangle config.el :results silent
...
*** Custom elisp functions (not used with hotkeys)
Evaluating a region or buffer, a tip from [[https://www.masteringemacs.org/article/evaluating-elisp-emacs][mastering Emacs]]. Not sure if I will keep using this one..
#+BEGIN_SRC emacs-lisp :tangle config.el :results silent
...
**** Play with roam directories
Kind of pointless
#+BEGIN_SRC emacs-lisp :results none
(defun my-three-roam-directories ()
  "Opens my three Roam directories; Desktop - Thinkpad - VBox_Arch"
    (interactive)
    (split-window-horizontally)
    (dired "~/Stack/Command_line/RoamNotes")
    (windmove-right)
    (split-window-horizontally)
    (dired "~/Stack/Thinkpad/RoamNotes")
    (windmove-right)
    (dired "~/Stack/VBox_Arch/RoamNotes"))
#+END_SRC
