...
A function to copy an org-table at point to the system clipboard in a csv format. Usefull for pasting to excel. The temporary file is saved in the RAM and the csv file will be gone after a reboot.
#+BEGIN_SRC emacs-lisp :tangle config.el :results silent
