...
Code snippets can automate lot's of stuff. Here below is and example. In org-mode <p tab will insert the following snippet:
...
Remark: github cannot handle a code block inside a codeblock. The line ~#+begin_src python :tangle ${2:wismij}.py :results output~ under the line ~# --~ is not showning on the github page. Also the bottom line ~#+end_sr${1:c}~ is not showing on the github page. Check the raw file for that.
