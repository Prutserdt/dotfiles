...
This part will refresh the packages after every reload, which is not needed actually and this part can be removed. I leave it here for a while and will remove if not needed.
#+begin_src emacs-lisp :tangle packages.el
;;(require 'package)
...
NOTE: if this part is placed at the start of ~config.el~ then transparency does not work. That's kinda weird.
#+begin_src emacs-lisp :tangle config.el
