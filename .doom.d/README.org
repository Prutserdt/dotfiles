...
This part will refresh the packages after every reload, which is not needed actuall and this part can be removed. I leave it here for a while and will remove if not needed.
#+begin_src emacs-lisp :tangle packages.el
;;(require 'package)
