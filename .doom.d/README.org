...
A Python mode hook to set the checker explicitly, see [[https://github.com/flycheck/flycheck/issues/185#issuecomment-22289167][this post]].
#+BEGIN_SRC emacs-lisp :tangle config.el :results silent
...
A useless pretty but Emacs welcome screen is created including an image and some text based on the default Doom emacs welcome screen.
#+BEGIN_SRC emacs-lisp :tangle config.el :results silent
