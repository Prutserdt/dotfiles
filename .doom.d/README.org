...
I want to use the escape key to exit keychords, similar to C-g. Let's see if this will give conflicts somewhere else....
#+BEGIN_SRC emacs-lisp :tangle config.el :results silent
...
** Custom code
*** Custom elisp functions (used with hotkeys)
...
Show a map of my asset allocation in time, a Python script.
#+BEGIN_SRC emacs-lisp :tangle config.el  :results silent
...
Show the current roam directory in the notification area of Emacs and show additional information as: the amount of org files, amount of total lines in all org files and word count in all files.
FIXME: I would like to include the information in the daily subdirectory, but did not get this to work yet...
#+BEGIN_SRC emacs-lisp :tangle config.el :results silent
